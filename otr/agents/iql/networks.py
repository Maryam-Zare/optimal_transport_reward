...
class Policy(hk.Module):
  ...
  def __call__(self,
    ...
    outputs = torso(
        observations,
        dropout_rate=self.dropout_rate if is_training else None,
        rng=key if is_training and self.dropout_rate is not None else None,
    )
...
def make_networks(
  ...
  def _critic_fn(o, a):
    a = jnp.clip(a, spec.actions.minimum, spec.actions.maximum)
